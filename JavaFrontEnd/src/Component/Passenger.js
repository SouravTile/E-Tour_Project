...
function PassengerDetail() {
  ...
  const getCost = () => {
    fetch("http://localhost:8081/api/cost_master/ById/" + pkgId)
      .then(response => response.json())
      .then(data => {
        setCost(data[0]); // Assuming the fetched data is a single object
        setSelectedOption(data?.key || (cost.singlePersonCost)); // Set default selected option
  ...
  const PostData = (async (passengerData) => {
    try {
      ...
      //const getresponse =async()=>{
        response  = await fetch("http://localhost:8081/api/passenger_master", {
        method: 'POST',
  ...
  return (
    ...
    <div className="user-input-container">
      ...
      <Button variant="primary" onClick={() =>  navigate(`/login/${pkgId}`)} className="submit-button">
                            Book My Tour
      </Button>
    </div>
    </div>

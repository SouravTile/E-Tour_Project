...
import './SearchDropdown.css'
  ...
  let navigate = useNavigate();
    ...
    const fetchDataByName = async () => {
      try {
        ...
        const response = await axios.get("https://localhost:7003/bycatId/"+searchName);
  ...
  const fetchDataByCost = async () => {
    try {
     console.log(searchCost);
      const response2 = await fetch("https://localhost:7003/cost/bycost/"+searchCost);
      const datahere = await response2.json();
      const catidd = datahere[0].catMasterId;
      const response4 = await fetch("https://localhost:7003/api/Category/"+catidd);
  ...
  const fetchAdditionalData = async (pkgId) => {
    try {
      ...
      const response = await axios.get("http://localhost:8081/api/package_master/"+pkgId);
  ...
  const fetchAdditionalData2 = async (catMasterId) => {
    try {
      ...
      const response1= await axios.get("http://localhost:8081/api/category_master/bycatMasterId/"+catMasterId);
  ...
  const fetchAdditionalData3 = async (catMasterId) => {
    try {
      ...
      const response2= await axios.get("http://localhost:8081/api/category_master/bycatMasterId/"+catMasterId);
  ...
  const handleDateSearch = async () => {
        try {
          ...
          const response = await axios.get("https://localhost:7003/date/bydate/"+startDate+"/"+endDate);
